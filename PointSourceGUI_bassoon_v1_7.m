...
%Timo Grothe, ETI, 13.03.2023

%% TODO 13.03.2023
%numbers output for Q and D
%reset button
%read from table
%stepped frequency slider

%last edit  16.05.2023: modified to reproduce the balloon plots in Fig. 6
% of the original submission

%% TODO 16.05.2023
%TODO: convert "bassoon_flow_Augustin_holes_open.mat" to a parameter table
%.csv
%buttons to switch between PS and CA model?
%file dialogue to load custom parameter tables (e.g. openwind)

%last edit  25.05.2023: modified to reproduce the three different models of the original submission
% PS, WG, WG+CA, switch between models by use of check boxes.

%% TODO 25.05.2023
%file dialogue to load custom parameter tables (e.g. openwind)
%default button for a simple point source array
%minor fixes (suggested by Jithin Thilakan)

%last edit  30.05.2023: added "reset to line array button", added "save to
%pdf" button

%last edit 05.09.2023 (Malte Kob): fixed paper position of figure, when
%exporting to *.fig. 
...
function varargout = PointSourceGUI_bassoon_v1_7(varargin)
% POINTSOURCEGUI_BASSOON_V1_7 MATLAB code for PointSourceGUI_bassoon_v1_7.fig
%      POINTSOURCEGUI_BASSOON_V1_7, by itself, creates a new POINTSOURCEGUI_BASSOON_V1_7 or raises the existing
%      singleton*.
%
%      H = POINTSOURCEGUI_BASSOON_V1_7 returns the handle to a new POINTSOURCEGUI_BASSOON_V1_7 or the handle to
%      the existing singleton*.
%
%      POINTSOURCEGUI_BASSOON_V1_7('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in POINTSOURCEGUI_BASSOON_V1_7.M with the given input arguments.
%
%      POINTSOURCEGUI_BASSOON_V1_7('Property','Value',...) creates a new POINTSOURCEGUI_BASSOON_V1_7 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PointSourceGUI_bassoon_v1_7_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PointSourceGUI_bassoon_v1_7_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PointSourceGUI_bassoon_v1_7

% Last Modified by GUIDE v2.5 20-Sep-2023 17:49:01

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @PointSourceGUI_bassoon_v1_7_OpeningFcn, ...
    'gui_OutputFcn',  @PointSourceGUI_bassoon_v1_7_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before PointSourceGUI_bassoon_v1_7 is made visible.
function PointSourceGUI_bassoon_v1_7_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PointSourceGUI_bassoon_v1_7 (see VARARGIN)

global nSources
nSources = 14;
% Choose default command line output for PointSourceGUI_bassoon_v1_7
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes PointSourceGUI_bassoon_v1_7 wait for user response (see UIRESUME)
...
function varargout = PointSourceGUI_bassoon_v1_7_OutputFcn(hObject, eventdata, handles)
...
%     warning on
    ...
    %% create new figure for pdf print
    fignew = figure('Visible','off','Units','normalized'); % Invisible figure
    cbAxes = findobj(handles.figure1,'Type','colorbar');  
    newAxes = copyobj([handles.axes1,cbAxes],fignew); % Copy the appropriate axess
    set(newAxes(1),'Units','normalized');
    set(newAxes(1),'Position',get(groot,'DefaultAxesPosition')); % The original position is copied too, so adjust it.
    set(fignew,'Position', [0.4183 0.3764 0.3366 0.5438])
    print(gcf,'-dpdf',filename)
    %%
...
if isempty(answer)
    ...
    %doesn't work: position of colobar is not copied correctly  
    fignew = figure('Visible','off','Units','normalized'); % Invisible figure
    cbAxes = findobj(handles.figure1,'Type','colorbar');  
    newAxes = copyobj([handles.axes1,cbAxes],fignew); % Copy the appropriate axess
    set(newAxes(1),'Units','normalized');
...
%    set(newAxes,'colormap',parula)
    set(fignew,'CreateFcn','set(gcbf,''Visible'',''on'')'); % Make it visible upon loading
    
%     %works 50%: add the toolbar now
%     cb = colorbar;
%     set(newAxes,'Position',get(groot,'DefaultAxesPosition')); 
%     %but now the figure needs to be adjusted (paperposition mode auto is not nice)
%     %set(cb,'Position',[0.988707025411061,0.11,0.047832585949178,0.815],'units','normalized');
...
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %% HELPER FUNCTION FOR .FIG EXPORT
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % taken from https://de.mathworks.com/matlabcentral/answers/234652-how-do-i-find-the-handle-to-the-colorbar-for-a-specific-axes
